...
class ReitanRating3(PowerLawRating):
    """Experimental multi-segment power law rating using the Reitan parameterization.
    ...
    def __init__(
        ...
        b1 = at.switch( h > hs, at.log(h - hs + ho), 0)
        #b = at.switch(h > hs1[1:], at.log(hs1[1:] - hs + ho), b1)
        b = at.switch(h > hs1[1:], at.log(hs1[1:] - hs), b1)
...
class ReitanRating2(PowerLawRating):
    """Experimental multi-segment power law rating using the Reitan parameterization.

    This version uses a parameterization that is more similar to the one used in Reitan Eq. 5.
    """
    def __init__(
        self,
        q,
        ...
        ho = pm.Uniform('ho', lower=0, upper=1, shape=(self.segments, 1))
        ...
        # clip hz
        hz = at.clip(ho*hs, 0, hs - 1)
        b1 = at.switch( h > hs, at.log(h - hz) - at.log(hs - hz), 0)
        b = at.switch(h > hs1[1:], at.log(hs1[1:] - hz) - at.log(hs - hz), b1)
